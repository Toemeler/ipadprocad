...
def main_for_test(bundle: str, label: str = "") -> int:
    """The whole program minus argv parsing.

    Separated so the tests can drive it directly. A CLI entry point that can
    only be exercised through the shell is a CLI entry point nobody tests, and
    these two scripts decide what every number in PERFORMANCE_PROFILE.md
    means.
    """
    d = load(bundle)
...
def main() -> int:  # pragma: no cover - argv plumbing
    ...
    a = ap.parse_args()
    try:
        return main_for_test(a.bundle, a.label)
