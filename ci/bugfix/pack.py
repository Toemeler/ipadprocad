...
# `accent` now has more than twenty — a field, nine value rows per palette, a
# re-tinting getter each, and the `T` façade — of which a colour change touches
# several. ~230 lines is a third of a cent; the round it saves is a nickel.
#
# CHOSEN ON A PLATEAU, not fitted. Sweeping theme.dart, every budget from 18 up
# shows the field, both palettes' value rows and both getter layers; 16 drops
# the getters and 8-10 drop them too. The sampler is not perfectly monotonic —
# a stride across a tier can land either side of a block — and rather than
# tune it against one file, the budget sits above the last dip, the same way
# BRIDGE_MAX_KEYS sits in the middle of its own plateau.
PINNED_SITES = 18
...
- Swift under `frontend/packages/*/ios/` CANNOT be compiled on Linux. Changing
  ...
  Two things ARE checked before your answer is accepted, so get them right:
  brackets must balance, and THE CHANNEL CONTRACT MUST LINE UP. If your Dart
  calls `invokeMethod('setAccent', {'light': x, 'dark': y})`, some Swift file
  in the same package's `ios/Classes/` must carry `case "setAccent":` and read
  `args["light"]` and `args["dark"]`. A name or a key spelled differently on
  the two sides compiles perfectly on BOTH and does nothing at all at runtime,
  which no test in this repository would catch.
- A UIKit surface cannot see a Dart `ValueNotifier`. When a value the native
  chrome PAINTS with changes, push it over the channel — `setAppearance`,
  `setViewportColor` and `setAccent` are the pattern — and send every variant
  the native side resolves between, not the one that happens to be active:
  UIKit picks by trait, so sending only the current colour is right until the
  next appearance switch and wrong after it.
- Never edit `.github/workflows/*`, `relay/*`, `ci/*`, or generated files under
  `lib/l10n/gen/` (those are regenerated by the build).
- A VALUE THAT IS STORED TWICE MUST BE CHANGED TWICE, and in this repo the
  duplication is invisible because it is baked into hex. Nine tokens in each
  palette ARE the accent at another alpha — `rawRibbonTop` at 0xD9,
  `rawCardHoverBorder` at full, `rawConActiveBg` at 0x2E — so "make the accent
  configurable" that changes only `accent` leaves the card highlight, the tab
  underline, the ribbon wash and the sketch nodes on the old colour. Before
  changing any colour, grep the palettes for its RGB and handle every row that
  shares it; `m236_theme_test` now fails the build when one is missed.
- AND OVERRIDE A VALUE WHERE IT LIVES, not at a reader. `T.x` is a façade over
  `Palette`, but `galleryChrome` hands widgets a `Palette` directly and
  `home_view` reads `g.cardHoverBorder` off it — so an override written into
  the `T` getters is simply skipped there. Anything memoised on the palette
  (`icon_theme.dart` caches on its identity) is skipped too.
