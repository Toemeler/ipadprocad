...
| Meilenstein | Stand |
|---|---|
| **M195** **Bug-Bundles gehen von selbst weg.** Der Knopf schrieb eine ZIP nach `Files > On My iPad > prototype > bugreports` — und dort lag sie, bis jemand daran dachte, sie zu verschicken. Ein Bericht, den man von Hand tragen muss, wird getragen, wenn es gerade passt, also nie in dem Moment, in dem der Fehler passiert ist. Neu: die App laedt jedes Bundle per **GitHub Contents API** (ein authentifizierter PUT, kein Server, kein Dienst) in ein Repository — bewusst DIESES Ziel, weil ein Repository der einzige Ort ist, auf den man auch einen Assistenten zeigen kann („schau dir die Bug-Reports an"), statt Postfach oder Chat-Webhook. **In diesem Repo ist nichts konfiguriert**: Ziel und Token stehen in `bugupload.json` im Dokumente-Ordner des GERAETS (ueber die Files-App erreichbar, nie in git). Keine Datei = kein Upload, und die App verhaelt sich exakt wie vorher — das ist der Default fuer jeden, der das hier klont. Das Ziel waehlt man beim Schreiben der Datei, nicht im Code: **dieses Repo ist OEFFENTLICH**, ein Bundle darin ist weltweit lesbar, ein separates privates Bug-Repo ist die empfohlene Adresse (und der Token laesst sich dann auf genau dieses eine Repo begrenzen). **Fehlschlag ist der Normalfall** auf einem iPad: ein nicht gesendetes Bundle BLEIBT und wird beim naechsten Bericht und beim naechsten Start erneut versucht; ein gesendetes bekommt eine `.sent`-Marke daneben statt geloescht zu werden (die lokale Kopie ist die einzige, die man auf dem Geraet oeffnen kann). Ein abgelehnter Token bricht den Durchlauf ab, statt zwanzig 401er hinterherzuschicken. **Zwei Wege, Unterschied ist NUR, wo das Geheimnis liegt:** *Relay* — die App PUTet die rohe ZIP an eine https-URL, die man selbst betreibt (irgendein Host, das Ende hier ist ein simpler PUT), der GitHub-Token liegt DORT serverseitig, auf dem Tablet liegt ein Upload-Key, mit dem man hoechstens Dateien ANHAENGEN kann; oder *direkt* an die Contents API mit einem Repo-Token auf dem Geraet. Ein direkter Token, der auf ein eigenes, sonst leeres Bug-Repo begrenzt ist, ist ungefaehr so gefaehrlich wie der Relay-Key — er darf nur nie auf den Quellcode zeigen. Setup: `BUGREPORTS.md` | ⚠️ **31 neue Tests geschrieben** (`m195_bug_upload_test.dart`; der HTTP-Aufruf ist injiziert, also wird jeder Status durchgespielt, den GitHub je antworten kann). Der wichtigste pinnt, dass **`toString()` den Token schwaerzt** — das Bundle traegt das ganze Log, ein verirrtes `$cfg` in einer Log-Zeile wuerde die Zugangsdaten also in genau die ZIP schreiben, die gleich hochgeladen wird. **EHRLICH: kein Flutter-SDK in dieser Sitzung** — gruen ist, was CI sagt |
