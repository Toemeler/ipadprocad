...
| **M125** **Drei Geraete-Meldungen aus derselben Sitzung, alle im 2D-Modus.** (1) **Trimmen eines Kreises mit zwei TANGENTEN nahm den ganzen Rand.** Beide Linien enden auf dem Rand; ihr Beruehrpunkt lag bei `t=1.000003` und fiel durch das feste 1e-9-Parameterfenster in `_segCircle`, waehrend die andere Tangente ZWEI Wurzeln 1e-5 auseinander lieferte — getrimmt wurde die Null-Spanne dazwischen, uebrig blieb ein Bogen von 1.7e-5 rad (im Log: `arc data=[…, 4.9872242, 4.9872410]`). Neu: die Diskriminante ist `4aa(r²-dPerp²)`, also ist alles innerhalb `2·r·1e-6` eine **Tangente = EIN** Beruehrpunkt, und die Segmentgrenze wird **radial** statt parametrisch geprueft (eine Radialabweichung von 1e-10 verschiebt die Wurzel um 9e-5 ENTLANG der Linie). Dazu `_distinct` gegen Doppel-Schnittpunkte. (2) **Punkt-auf-Kreis fehlte am Linienende.** Die Bindung haing an der Zeichenreihenfolge: `inferPointBindings` fragte nur „landet ein Punkt des NEUEN Objekts auf etwas Aelterem". Im Log wurde die Linie zuerst gezeichnet, der Kreis danach DURCH ihren Endpunkt. Neu bindet die Inferenz auch rueckwaerts (bestehender Punkt auf die neue Kurve), durch dasselbe Overconstrain-Tor wie eine manuelle Bedingung. (3) **Trimmen erhaelt das Original als Konstruktionsgeometrie** (ausser es war schon Konstruktion): der Traeger bleibt an Ort und Stelle, **kein Constraint wird mehr umgehaengt oder geloescht** — Masse und Bedingungen treiben weiter —, und die Stuecke werden per Punkt-auf-Punkt / Punkt-auf-Kurve (+ `equal` bei Boegen) an ihn gebunden. Ein Schnittpunkt ist damit durch Traeger ∩ Schneider voll bestimmt; das alte explizite Punkt-auf-Punkt gestapelter Schnittecken ist redundant geworden und faellt sauber durchs Tor. Picken bevorzugt bei Gleichstand die normale Geometrie ueber dem Ghost darunter | ✅ Host: **666 Tests gruen** (13 neu, `m125_trim_and_binds_test.dart`; 9 alte Trim-Erwartungen auf den neuen Kontrakt gezogen), analyze 51 Issues / 0 errors = Ausgangsstand. Belegt an den Zahlen aus `bug20260804T112835` (Kreis r8.2223 + 2 Tangenten: Bogen > 1 rad statt 1.7e-5) und `T112452`. **Geraete-Test offen** |
