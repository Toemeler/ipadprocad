...
| **M64** Galerie-/Kontextmenue-Vorschau kommt aus DERSELBEN RealityKit-Engine wie der 3D-Viewport (Off-Screen-ARView ueber den neuen Plugin-Kanal `prototype/reality_view/thumb`, CPU-Painter nur noch als Fallback) und steht **immer auf der Ecke oben-vorne-rechts** (`kThumbPol = acos(1/sqrt 3)` statt gerundet 0.955, dieselbe `fitThumbCamera` fuer beide Engines) | ⚠️ implementiert, **ungeprueft**: in der Session gab es weder Flutter/Dart noch Swift — `flutter analyze`, Host-Suite und Swift-Kompilat stehen aus; Host-Test `m64_thumb_engine_test.dart` geschrieben, Geraete-Test offen |
