...
| **M125** **Die Skizze der ZWEITEN Extrusion entstand unter dem End of Part.** `commitExtrude` parkte die Marke mit `eopAfter = partTimeline(p).length` — die richtige Position in genau dieser Millisekunde und der falsche WERT, denn der Zeitstrahl waechst. Nach der ersten Extrusion ist das Teil EINE Zeile lang (die Skizze ist konsumiert und nistet unter ihrem Feature), die Marke stand also auf 1, und die naechste Skizze wurde als Zeile 1 angehaengt: unter dem Balken, ausgegraut, in 3D nicht gezeichnet — waehrend ihr Editor bereits offen war. Neu ist der Sentinel `kEopAtEnd`: "am Ende" ist nie eine Zeilenzahl (das Feld hatte ihn als Default laengst, er ging nur bei der ersten Extrusion verloren). Jeder Konsument klemmt ohnehin auf die Zeilenzahl, also aendert sich nichts ausser dass die Marke mitwaechst; persistiert wird weiter nur ein echtes Parken. Vier Setzer heissen jetzt `endOfPartToEnd()`, dazu zwei Stellen mit demselben Muster: `loadJson` gab einem Teil ohne gespeicherte Marke die Zeilenzahl (ein gespeichertes Teil war also schon beim OEFFNEN betroffen, nicht erst nach der ersten Extrusion), und `setEndOfPart` speicherte einen Zug bis ganz nach unten als "Zeile n" statt als "am Ende". Neu `_admitNewTimelineRow`: steht die Marke ECHT mitten im Baum, rueckt sie fuer eine neue Skizze ans Ende und laesst die Arbeit zu — genau das tat `commitExtrude` fuer ein neues Feature seit M91, die Skizzen-Pfade hielten sich nur nicht daran | ✅ Host: **661 Tests gruen** (8 neu, `m125_eop_floats_test.dart`), `flutter analyze` 51 Issues / **0 errors** = exakt der Ausgangsstand; lokal mit Flutter 3.32.0. Gegen den unreparierten Stand laufen 5 der 8 neu roten Tests, der erste mit `Expected: empty / Actual: ['Sketch2']` — woertlich die Meldung. Lehrreich: die DRITTE Runde war auch ohne Fix gruen, weil `commitExtrude` am Ende jeder vollstaendigen Runde die Zahl wieder passend setzt — sichtbar ist der Fehler nur ZWISCHEN den Runden, also waehrend man in der neuen Skizze sitzt. **Geraete-Test offen** |
