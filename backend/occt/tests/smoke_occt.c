 ...
 *
 * v26 scenario — S16, the straight audit:
 *   [39] EVERY direction, axis and placement parameter in this shim, bent
 ...
 *        UI. [39a] a real rotation through occt_transform (the only rotation
 *        this file had was {0,-1,0, 1,0,0, 0,0,1}, exact 0/±1 entries);
 *        [39b] revolve about an oblique, off-origin axis, against PAPPUS
 *        rather than a golden; [39c] a hole revolved about that same axis —
 *        the control for S14's item 2; [39d] the coil's `clockwise` flag,
 *        which nothing had ever set; [39e] the coil about an oblique axis;
 *        [39f] an oblique mirror plane; [39g] a boolean with a ROTATED
 *        operand; [39h] loft with rotated section placements; [39i] a face
 *        moved along an OBLIQUE delta; [39j] the chamfer's `angle >= 90`
 *        guard on a 60° edge; [39k] a fillet on a 135° edge; [39l] chamfer
...
int main(void)
{
     ...
     * ==================================================================== */
    {
        ...
        /* ---- [39a] P5 — occt_transform ACCEPTS a real rotation ---------- */
        {
            ...
            det = R[0]*(R[5]*R[10] - R[6]*R[9])
                ...
                + R[2]*(R[4]*R[9]  - R[5]*R[8]);
            printf("[39a] Rodrigues residual: orthonormality %.3e, |det-1| "
                   "%.3e (guard is 1e-9)\n", worst, fabs(det - 1.0));
            check(worst < 1e-12 && fabs(det - 1.0) < 1e-12,
                  "[39a] a hand-composed rotation is not orthonormal to 1e-12 "
                  "— the 1e-9 guard has less headroom than it looks");

            occt_shape *b = occt_make_box(10, 20, 30);
            occt_shape *rb = b ? occt_transform(b, R) : NULL;
            if (check(rb != NULL,
                      "[39a] occt_transform REFUSED a genuine rotation")) {
                check(near_rel(occt_shape_volume(rb), 6000.0, 1e-12),
                      "[39a] a rotation changed the volume");
                check(occt_shape_valid(rb), "[39a] rotated box invalid");
        ...
        /* ---- [39b] P3 — REVOLVE about an oblique, off-origin axis ------- */
        {
            ...
            occt_shape *s = occt_revolve_profile(P, lc, 1, -4.0, 1.0,
                                                 0.6, 0.8, 360.0);
            if (check(s != NULL,
                      "[39b] revolve about an oblique axis returned NULL")) {
                const double v = occt_shape_volume(s);
                printf("[39b] oblique-axis revolve %.6f (Pappus %.6f)\n",
                       v, want);
                check(near_rel(v, want, 1e-6),
                      "[39b] oblique revolve misses Pappus");
                check(occt_shape_valid(s), "[39b] oblique revolve invalid");
                occt_free_shape(s);
            }
            ...
            occt_shape *r2 = occt_revolve_profile(P, lc, 1, -4.0, 1.0,
                                                  -0.6, -0.8, 360.0);
            if (check(r2 != NULL, "[39b] reversed-axis revolve returned NULL")) {
                const double v = occt_shape_volume(r2);
                printf("[39b] reversed-axis revolve %.6f (want %.6f)\n",
                       v, want);
                check(near_rel(v, want, 1e-6),
                      "[39b] reversing the axis direction changed the solid");
                check(occt_shape_valid(r2),
                      "[39b] reversed-axis revolve invalid");
                occt_free_shape(r2);
            }
            ...
            occt_shape *q = occt_revolve_profile(P, lc, 1, -4.0, 1.0,
                                                 0.6, 0.8, 90.0);
            if (check(q != NULL, "[39b] partial oblique revolve NULL")) {
                check(near_rel(occt_shape_volume(q), want / 4.0, 1e-6),
                      "[39b] a quarter turn is not a quarter of the material");
                occt_free_shape(q);
            }
        }

        /* ---- [39c] P4 — a HOLE revolved about the SAME oblique axis ----- */
        {
            ...
            occt_shape *s = occt_revolve_profile(P, lc, 2, -4.0, 1.0,
                                                 0.6, 0.8, 360.0);
            if (check(s != NULL, "[39c] holed oblique revolve returned NULL")) {
                const double v = occt_shape_volume(s);
                printf("[39c] holed oblique revolve %.6f (Pappus %.6f)\n",
                       v, want);
                check(near_rel(v, want, 1e-6),
                      "[39c] the hole is not on the body's own axis");
                check(occt_shape_valid(s), "[39c] holed oblique revolve invalid");
                occt_free_shape(s);
            }
        }

        /* ---- [39d] P1 — the COIL's `clockwise` flag --------------------- */
        {
            ...
            if (check(ccw != NULL && cw != NULL,
                      "[39d] a coil returned NULL")) {
                double b0[6], b1[6];
                const double v0 = occt_shape_volume(ccw);
                const double v1 = occt_shape_volume(cw);
                occt_bbox(ccw, b0);
                occt_bbox(cw, b1);
                printf("[39d] ccw vol %.4f z[%.4f,%.4f] | cw vol %.4f "
                       "z[%.4f,%.4f] (rise asked for: 50)\n",
                       v0, b0[2], b0[5], v1, b1[2], b1[5]);
                check(near_rel(v0, want, 2e-2), "[39d] ccw coil volume far off");
                check(near_rel(v1, want, 2e-2), "[39d] cw coil volume far off");
        ...
        /* ---- [39e] P2 — the COIL about an oblique axis (equivariance) --- */
        {
            ...
            if (check(base != NULL && rot != NULL,
                      "[39e] the coil refused an oblique axis")) {
                const double v0 = occt_shape_volume(base);
                const double v1 = occt_shape_volume(rot);
                printf("[39e] coil equivariance: +Z %.9f, oblique %.9f, "
                       "rel diff %.3e\n", v0, v1, fabs(v1 - v0) / v0);
                check(near_rel(v1, v0, 1e-9),
                      "[39e] the coil is not equivariant — an axis is baked in");
                check(occt_shape_valid(rot), "[39e] oblique coil invalid");
            }
            occt_free_shape(base);
            occt_free_shape(rot);
        }

        /* ---- [39f] P6 — MIRROR about an oblique plane ------------------- */
        {
            ...
            if (check(mir != NULL, "[39f] oblique mirror returned NULL")) {
                check(near_rel(occt_shape_volume(mir), 6000.0, 1e-9),
                      "[39f] an oblique mirror changed the volume");
                check(occt_shape_valid(mir), "[39f] oblique mirror invalid");
            ...
            if (check(m3 != NULL, "[39f] un-normalised normal was refused")) {
                double b1[6], b2[6];
                ...
                if (m1 != NULL && occt_bbox(m1, b1) && occt_bbox(m3, b2)) {
                    ...
                    for (k = 0; k < 6; ++k)
                        if (!near_rel(b1[k], b2[k], 1e-9) &&
                            ...
                            same = 0;
                    check(same,
                          "[39f] scaling the plane normal changed the mirror");
                }
                occt_free_shape(m1);
                occt_free_shape(m3);
            }
        ...
        /* ---- [39g] P7 — a BOOLEAN with a ROTATED operand ---------------- */
        {
            ...
            if (check(cut != NULL, "[39g] cut with a rotated tool returned NULL")) {
                ...
                if (check(cutR != NULL, "[39g] the rotated-frame cut failed")) {
                    const double v1 = occt_shape_volume(cutR);
                    printf("[39g] boolean equivariance: %.9f vs %.9f, rel diff "
                           "%.3e\n", v0, v1, fabs(v1 - v0) / v0);
                    check(near_rel(v1, v0, 1e-6),
                          "[39g] the boolean is not equivariant under a rigid "
                          "motion");
                    check(occt_shape_valid(cutR), "[39g] rotated-frame cut invalid");
                }
                check(occt_shape_valid(cut), "[39g] rotated-tool cut invalid");
        ...
        /* ---- [39h] P8 — LOFT with ROTATED section placements ------------ */
        {
            ...
            if (check(lo != NULL,
                      "[39h] loft with rotated section matrices returned NULL")) {
                const double v = occt_shape_volume(lo);
                printf("[39h] rotated-frame loft %.9f (want 2500)\n", v);
                check(near_rel(v, 2500.0, 1e-9),
                      "[39h] the loft is not equivariant — a section frame is "
                      "composed against a baked-in axis");
                check(occt_shape_valid(lo), "[39h] rotated-frame loft invalid");
                occt_free_shape(lo);
            }
             ...
             * sections. Registered as a weak claim on purpose (S16 P8). */
            {
                ...
                if (check(tl != NULL, "[39h] tilted-section loft returned NULL")) {
                    ...
                    printf("[39h] tilted-section loft %.6f (bracket %.6f .. "
                           "%.6f)\n", v, lo_b, hi_b);
                    check(v > lo_b && v < hi_b,
                          "[39h] the tilted loft is outside its bracket");
                    check(occt_shape_valid(tl), "[39h] tilted loft invalid");
                    occt_free_shape(tl);
                }
            }
        }

        /* ---- [39i] P9 — MOVE FACES with an OBLIQUE delta ---------------- */
        {
            ...
            if (check(top > 0, "[39i] could not find the top face")) {
                const int ids[1] = {top};
                occt_shape *ob = occt_move_faces(box, ids, 1, 5.0, 0.0, 5.0);
                if (check(ob != NULL,
                          "[39i] an oblique move was neither refused nor built")) {
                    double hits[8];
                    const double v = occt_shape_volume(ob);
                    const int n = occt_ray_hits(ob, 2.0, 10.0, -10.0,
                                                0.0, 0.0, 1.0, hits, 8);
                    printf("[39i] oblique move volume %.6f (perpendicular move "
                    ...
                    check(near_rel(v, 10000.0, 1e-6),
                          "[39i] volume is not the swept-slab answer");
                    check(occt_shape_valid(ob),
                          "[39i] the oblique move is invalid");
                    ...
                    if (check(n >= 2, "[39i] the ray missed the moved body")) {
                        const double exit_z = hits[n - 1] - 10.0;
                        check(fabs(exit_z - 22.0) < 1e-6,
                              "[39i] the oblique move no longer leans — if the "
        ...
        /* ---- [39j] P10 — the chamfer's `angle >= 90` guard -------------- */
        {
            ...
            if (check(wedge != NULL, "[39j] wedge prism returned NULL")) {
                ...
                printf("[39j] wedge volume %.6f (want %.6f), 60-degree edge "
                ...
                check(near_rel(occt_shape_volume(wedge),
                      ...
                      "[39j] the wedge prism is not area x height");
                check(e60 > 0,
                      "[39j] an equilateral prism must have a 60-degree "
            ...
            if (e60 > 0) {
                ...
                printf("[39j] theta=60 edge: mode2 alpha=80 -> %s | mode2 "
                ...
                check(ok80 != NULL,
                      "[39j] mode 2 must work on a 60-degree edge at all");
                check(ref100 == NULL,
                      "[39j] the >= 90 guard did not fire — if this starts "
                      "failing the guard was relaxed and this scenario should "
                      "assert the new range instead");
                if (check(eq != NULL,
                          "[39j] the two-distance spelling of the SAME chamfer "
                    ...
                    printf("[39j] mode1 removed %.6f (analytic %.6f) — a "
                           "chamfer the mode2 spelling REFUSES\n", cut, got);
                    check(near_rel(got, cut, 1e-6),
                          "[39j] the two-distance chamfer is not the triangle "
                          "1/2 d1 d2 sin(theta)");
                    check(occt_shape_valid(eq), "[39j] that chamfer is invalid");
                }
        ...
        /* ---- [39k] P11 — FILLET a 135-degree edge ----------------------- */
        {
            ...
            if (check(ch != NULL, "[39k] setup chamfer failed")) {
                ...
                printf("[39k] 135-degree edges on the chamfered cube: %d\n",
                       n135);
                check(n135 == 2,
                      "[39k] a 45-degree chamfer must leave two 135-degree "
                      "edges");
                if (e135 > 0) {
            ...
            /* ---- [39l] P12 — CHAMFER MODE 1, which nothing has ever called */
            if (vertical > 0) {
                ...
                if (check(c42 != NULL && c24 != NULL,
                          "[39l] two-distance chamfer (mode 1) failed — it has "
                          "never had a fixture")) {
                    check(near_rel(occt_shape_volume(c42), 7920.0, 1e-9),
                          "[39l] mode 1 does not remove d1*d2/2 * length");
                    check(near_rel(occt_shape_volume(c24), 7920.0, 1e-9),
                          "[39l] the swapped pair removes a different amount");
                    check(occt_shape_valid(c42) && occt_shape_valid(c24),
                          "[39l] a two-distance chamfer is invalid");
                    ...
                    if (check(w42 && w24 && occt_bbox(w42, b42) &&
                              occt_bbox(w24, b24),
                              "[39l] could not isolate the wedges")) {
                        double e42[3], e24[3];
                        int k, differ = 0;
                        double sorted[3];
                        for (k = 0; k < 3; ++k) {
                        ...
                        printf("[39l] wedge extents 4/2 (%.4f %.4f %.4f) vs "
                               "2/4 (%.4f %.4f %.4f)\n",
                               e42[0], e42[1], e42[2],
                               e24[0], e24[1], e24[2]);
                        check(near_rel(sorted[0], 2.0, 1e-6) &&
                              ...
                              "[39l] the wedge is not d1 x d2 x length");
                        check(differ,
                              "[39l] swapping d1 and d2 changed nothing — the "
