 ...
 *
 * Build:  cmake -S backend/occt -B build -DOCCT_SMOKE=ON -DCMAKE_PREFIX_PATH=<occt>
 *         cmake --build build --target occt_mesh_recon_test && ./build/occt_mesh_recon_test
 */
...
{
    if (ok) { passes++; }
    else { fails++; std::printf("   FAIL: %s %s\n", what, extra.c_str()); }
}
...
{
    BRepMesh_IncrementalMesh mesher(s, defl, Standard_False, 0.3, Standard_True);
    xyz.clear(); tri.clear();
    for (TopExp_Explorer ex(s, TopAbs_FACE); ex.More(); ex.Next()) {
        ...
        for (int i = 1; i <= t->NbNodes(); ++i) {
            ...
            xyz.push_back(p.X()); xyz.push_back(p.Y()); xyz.push_back(p.Z());
        }
...
static double MeshVolume(const std::vector<double>& xyz, const std::vector<int>& tri)
{
    double v = 0;
...
struct Counts { int planes = 0, cyl = 0, cone = 0, sph = 0, tor = 0, other = 0, total = 0; };
static Counts FaceKinds(const TopoDS_Shape& s)
{
    ...
    for (TopExp_Explorer ex(s, TopAbs_FACE); ex.More(); ex.Next()) {
        ...
        c.total++;
        switch (sa.GetType()) {
            case GeomAbs_Plane: c.planes++; break;
            case GeomAbs_Cylinder: c.cyl++; break;
            case GeomAbs_Cone: c.cone++; break;
            case GeomAbs_Sphere: c.sph++; break;
            case GeomAbs_Torus: c.tor++; break;
...
static void report(const meshrecon::Report& r)
{
    std::printf("   tri %d->%d  vtx %d->%d  patches %d  "
                ...
                "rms %.5f  solid %d\n",
                r.triangles_in, r.triangles_used, r.vertices_in, r.vertices_welded,
                r.patches, r.planes, r.cylinders, r.cones, r.spheres, r.tori,
                r.faceted_patches, r.faces_built, r.faces_failed,
                r.analytic_edges, r.approximated_edges, r.fit_rms, r.closed);
}
                        ...
                        meshrecon::Report& rep, int mode = 1)
{
    ...
    TopoDS_Shape out = meshrecon::Reconstruct(
        xyz.data(), (int)(xyz.size() / 3), tri.data(), (int)(tri.size() / 3),
        p, rep, err);
    if (out.IsNull()) std::printf("   (null: %s)\n", err.c_str());
    return out;
}
...
int main()
{
    ...
    // ---- 2. cylinder ----------------------------------------------------
    {
        ...
        if (!out.IsNull()) {
            ...
            chk("2 planes + 1 cylinder", c.planes == 2 && c.cyl == 1,
                std::to_string(c.planes) + "pl " + std::to_string(c.cyl) + "cy " +
    ...
    // ---- 6. inside-out box ----------------------------------------------
    {
        ...
        for (size_t i = 0; i < tri.size(); i += 3) std::swap(tri[i + 1], tri[i + 2]);
        meshrecon::Params p = meshrecon::Defaults();
        meshrecon::Report r; std::string err;
        TopoDS_Shape out = meshrecon::Reconstruct(
            xyz.data(), (int)(xyz.size() / 3), tri.data(), (int)(tri.size() / 3),
            p, r, err);
        report(r);
        ...
        if (!out.IsNull()) {
            chk("volume is POSITIVE 1000", std::fabs(Volume(out) - 1000.) < 1e-6,
                std::to_string(Volume(out)));
        }
    }
    // ---- 7. faceted mode -------------------------------------------------
    {
        ...
        if (!out.IsNull()) {
            chk("volume 1000", std::fabs(Volume(out) - 1000.) < 1e-6,
                std::to_string(Volume(out)));
            Counts c = FaceKinds(out);
            chk("coplanar merge got it to 6", c.total == 6, std::to_string(c.total));
        }
    }
    // ---- 8. filleted block (the hard one) --------------------------------
    {
        ...
        for (TopExp_Explorer ex(box, TopAbs_EDGE); ex.More(); ex.Next()) {
            ...
            if (std::fabs(p1.Z() - p2.Z()) > 1e-6) fil.Add(3., e);
        }
        TopoDS_Shape src;
        try { src = fil.Shape(); } catch (...) {}
        if (src.IsNull()) { std::printf("   (fillet failed, skipped)\n"); }
        else {
            meshrecon::Report r;
            ...
            if (!out.IsNull()) {
                ...
                chk("volume tracks the mesh",
                    ...
                    std::to_string(g_meshVolume) + " src " + std::to_string(Volume(src)));
                chk("valid", BRepCheck_Analyzer(out).IsValid());
            }
        }
    }
    ...
    for (int i = 0; i < 5; ++i)
        for (int j = 0; j < 3; ++j) {
            gp_Ax2 ax(gp_Pnt(15. + i * 22.5, 15. + j * 25., -1.), gp_Dir(0,0,1));
            part = BRepAlgoAPI_Cut(part, BRepPrimAPI_MakeCylinder(ax, 4., 12.).Shape()).Shape();
        }
    for (int i = 0; i < 3; ++i) {
        gp_Ax2 ax(gp_Pnt(25. + i * 35., 45., 10.), gp_Dir(0,0,1));
        part = BRepAlgoAPI_Fuse(part, BRepPrimAPI_MakeCylinder(ax, 6., 14.).Shape()).Shape();
    }
    int srcFaces = 0;
    for (TopExp_Explorer ex(part, TopAbs_FACE); ex.More(); ex.Next()) srcFaces++;
    GProp_GProps gs; BRepGProp::VolumeProperties(part, gs);
    ...
    for (double defl : {0.2, 0.05, 0.01}) {
        ...
        TopoDS_Shape out = meshrecon::Reconstruct(xyz.data(), (int)(xyz.size()/3),
        ...
        double vol = 0;
        if (!out.IsNull()) { GProp_GProps g; BRepGProp::VolumeProperties(out, g); vol = g.Mass(); }
        std::printf("defl %.2f: %7d tri -> %3d faces (src %d)  %6.0f ms  "
                    "patches %d pl %d cy %d built %d failed %d faceted %d  solid %d  "
                    "vol %.1f (src %.1f)  %s\n",
                    defl, (int)(tri.size()/3), nf, srcFaces, ms,
                    r.patches, r.planes, r.cylinders, r.faces_built,
                    r.faces_failed, r.faceted_patches, r.closed,
                    vol, gs.Mass(), err.c_str());
    ...
    // ---- robustness: input that is broken the way downloads are broken ----
    {
        ...
        std::vector<double> soup; std::vector<int> stri;
        for (size_t i = 0; i < tri.size(); ++i) {
            soup.push_back(xyz[tri[i]*3]); soup.push_back(xyz[tri[i]*3+1]);
            soup.push_back(xyz[tri[i]*3+2]);
            stri.push_back((int)i);
        }
        meshrecon::Report r2;
        TopoDS_Shape out = meshrecon::Reconstruct(soup.data(), (int)(soup.size()/3),
                                                  stri.data(), (int)(stri.size()/3),
                                                  p, r2, err);
        chk("unwelded soup welds and closes", r2.closed == 1,
            std::to_string(r2.vertices_in) + "->" + std::to_string(r2.vertices_welded));
