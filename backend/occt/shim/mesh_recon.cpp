...
void ClearReport(Report& r) { std::memset(&r, 0, sizeof(r)); }
...
inline gp_Pnt P(const V3& a) { return gp_Pnt(a.x, a.y, a.z); }
inline gp_Dir D(const V3& a) { return gp_Dir(a.x, a.y, a.z); }
...
{
    for (int i = 0; i < 9; ++i) vec[i] = (i % 4 == 0) ? 1.0 : 0.0;
    for (int sweep = 0; sweep < 24; ++sweep) {
        double off = 0;
        for (int i = 0; i < 3; ++i)
            for (int j = i + 1; j < 3; ++j) off += m[i * 3 + j] * m[i * 3 + j];
        if (off < 1e-30) break;
        for (int p = 0; p < 3; ++p) {
            for (int q = p + 1; q < 3; ++q) {
    ...
    for (int i = 0; i < 3; ++i) {
        val[i] = d[order[i]];
        for (int k = 0; k < 3; ++k) out[i * 3 + k] = vec[k * 3 + order[i]];
    }
    std::memcpy(vec, out, sizeof(out));
}

/* Gauss elimination with partial pivoting. n <= 8. Returns false if singular. */
...
struct Mesh {
    ...
    /* adj[t*3+k] is the triangle across the edge (tri[t*3+k], tri[t*3+(k+1)%3]),
     * or -1 at a boundary or a non-manifold edge. */
    std::vector<int> adj;
    V3 bbmin, bbmax;
    double diagonal = 0;
               ...
               const Params& prm, Mesh& m, Report& rep)
{
    ...
    for (int i = 0; i < nv; ++i) {
        ...
        m.bbmin.x = std::min(m.bbmin.x, p.x); m.bbmax.x = std::max(m.bbmax.x, p.x);
        m.bbmin.y = std::min(m.bbmin.y, p.y); m.bbmax.y = std::max(m.bbmax.y, p.y);
        m.bbmin.z = std::min(m.bbmin.z, p.z); m.bbmax.z = std::max(m.bbmax.z, p.z);
    }
    m.diagonal = Norm(m.bbmax - m.bbmin);
    if (!(m.diagonal > 0)) return false;
    ...
    for (int t = 0; t < nt; ++t) {
        ...
        if (Norm(n) <= 1e-24 * m.diagonal * m.diagonal) continue;
        m.tri.push_back(ra); m.tri.push_back(rb); m.tri.push_back(rc);
    }
    rep.triangles_used = m.triCount();
    if (m.triCount() < 1) return false;
...
void FlipTriangle(Mesh& m, int t)
{
    ...
    if (!m.adj.empty()) std::swap(m.adj[t * 3 + 0], m.adj[t * 3 + 2]);
...
void BuildAdjacency(Mesh& m, Report& rep)
{
    ...
    for (int t = 0; t < m.triCount(); ++t) {
        for (int k = 0; k < 3; ++k) {
            ...
            const long long key = (long long)std::min(a, b) * n + std::max(a, b);
            ...
            if (c == 1) {
                ...
                if (m.adj[other] >= 0) {
                    ...
                    for (int kk = 0; kk < 3; ++kk)
                        if (m.adj[o2 * 3 + kk] == other / 3) m.adj[o2 * 3 + kk] = -1;
                    m.adj[other] = -1;
                }
                m.adj[t * 3 + k] = -1;
            }
    ...
    for (auto& e : count) {
        if (e.second == 1) rep.boundary_edges++;
        else if (e.second > 2) rep.non_manifold_edges++;
    }
}
...
void OrientMesh(Mesh& m, Report& rep)
{
    ...
    for (int seed = 0; seed < nt; ++seed) {
        ...
        stack.push_back(seed); seen[seed] = 1;
        while (!stack.empty()) {
            ...
            for (int k = 0; k < 3; ++k) {
                ...
                bool same = false;
                for (int kk = 0; kk < 3; ++kk) {
                    if (m.tri[o * 3 + kk] == a && m.tri[o * 3 + (kk + 1) % 3] == b) {
                        same = true; break;
                    }
                }
                if (same) {
...
enum SurfKind { kNone = 0, kPlane, kCylinder, kCone, kSphere, kTorus,
                kFreeform, kFaceted };
...
{
    switch (k) {
        case kPlane: return 4;
        case kSphere: return 4;
        case kCylinder: return 7;
        case kCone: return 7;
        case kTorus: return 8;
        default: return 0;
    }
}
 ...
 *
 * Two wrong versions of this got written before the right one, and both failures
 ...
 * surface, and a normal derived from it cannot drift out of agreement with it. */
V3 SurfNormal(SurfKind k, const double* q, const V3& p, double h)
{
    const V3 g(
        SurfDist(k, q, V3(p.x + h, p.y, p.z)) - SurfDist(k, q, V3(p.x - h, p.y, p.z)),
        SurfDist(k, q, V3(p.x, p.y + h, p.z)) - SurfDist(k, q, V3(p.x, p.y - h, p.z)),
        SurfDist(k, q, V3(p.x, p.y, p.z + h)) - SurfDist(k, q, V3(p.x, p.y, p.z - h)));
    return Unit(g);
}
               ...
               const bool* freeMask = nullptr)
{
    ...
    if (n == 0 || pts.size() < static_cast<size_t>(n)) return;
    ...
    for (int iter = 0; iter < 24; ++iter) {
        ...
        for (const V3& p : pts) {
            ...
            for (int i = 0; i < n; ++i) {
                if (freeMask && !freeMask[i]) { J[i] = 0; continue; }
            ...
            for (int i = 0; i < n; ++i) {
                Jtr[i] -= J[i] * r;
                for (int j = 0; j < n; ++j) JtJ[i * n + j] += J[i] * J[j];
            }
        }
        double A[64], b[8];
        ...
        for (int i = 0; i < n; ++i) {
            if (freeMask && !freeMask[i]) { A[i * n + i] = 1.0; continue; }
            A[i * n + i] *= (1.0 + lambda);
        }
        if (!SolveLin(A, b, n)) break;
        double cand[8];
        std::memcpy(cand, q, sizeof(cand));
        for (int i = 0; i < n; ++i) cand[i] += b[i];
        Renormalise(k, cand);
        if (freeMask) {
            for (int i = 0; i < n; ++i)
                if (!freeMask[i]) cand[i] = q[i];
        }
...
{
    c = V3();
    for (const V3& p : pts) c += p;
    if (!pts.empty()) c = c * (1.0 / pts.size());
}
...
void Covariance(const std::vector<V3>& pts, const V3& c, double m[9])
{
    ...
    for (const V3& p : pts) {
        ...
        for (int i = 0; i < 3; ++i)
            for (int j = 0; j < 3; ++j) m[i * 3 + j] += v[i] * v[j];
    }
}
...
bool SeedPlane(const std::vector<V3>& pts, double* q)
{
    ...
    q[0] = n.x; q[1] = n.y; q[2] = n.z; q[3] = Dot(n, c);
    return true;
}
...
bool SeedSphere(const std::vector<V3>& pts, double* q)
{
    ...
    for (const V3& p : pts) {
        ...
        for (int i = 0; i < 4; ++i) {
            b[i] += row[i] * rhs;
            for (int j = 0; j < 4; ++j) A[i * 4 + j] += row[i] * row[j];
        }
    }
    if (!SolveLin(A, b, 4)) return false;
    const double r2 = b[3] + b[0] * b[0] + b[1] * b[1] + b[2] * b[2];
    if (!(r2 > 0)) return false;
    q[0] = b[0]; q[1] = b[1]; q[2] = b[2]; q[3] = std::sqrt(r2);
    return true;
}
...
bool SeedCylinder(const std::vector<V3>& pts, const std::vector<V3>& nrm, double* q)
{
    if (pts.size() < 5 || nrm.size() != pts.size()) return false;
    ...
    for (const V3& p : pts) {
        ...
        for (int i = 0; i < 3; ++i) {
            b[i] += row[i] * rhs;
            for (int j = 0; j < 3; ++j) A[i * 3 + j] += row[i] * row[j];
        }
    }
    if (!SolveLin(A, b, 3)) return false;
    const double r2 = b[2] + b[0] * b[0] + b[1] * b[1];
    if (!(r2 > 0)) return false;
    const V3 axisPt = c + e1 * b[0] + e2 * b[1];
    q[0] = axisPt.x; q[1] = axisPt.y; q[2] = axisPt.z;
    q[3] = a.x; q[4] = a.y; q[5] = a.z;
    q[6] = std::sqrt(r2);
    return true;
}
...
{
    if (pts.size() < 6 || nrm.size() != pts.size()) return false;
    double pl[4];
    if (!SeedPlane(nrm, pl)) return false;
    V3 a = Unit(V3(pl[0], pl[1], pl[2]));
    double sinA = pl[3];
    if (sinA < 0) { a = a * -1.0; sinA = -sinA; }
    if (!(sinA > 1e-3) || sinA > 0.999) return false;   /* cylinder or plane */
    ...
    for (size_t i = 0; i < pts.size(); ++i) {
        ...
        A[0] += lx * lx; A[1] += lx * ly;
        A[2] += ly * lx; A[3] += ly * ly;
        b[0] += lx * off; b[1] += ly * off;
    }
    if (!SolveLin(A, b, 2)) return false;
    const V3 axisPt = c + e1 * b[0] + e2 * b[1];
    ...
    if (std::fabs(slope) < 1e-9) return false;
    ...
    q[0] = apex.x; q[1] = apex.y; q[2] = apex.z;
    q[3] = dir.x; q[4] = dir.y; q[5] = dir.z;
    q[6] = alpha;
    return true;
}
...
bool SeedTorus(const std::vector<V3>& pts, const std::vector<V3>& nrm, double* q)
{
    const size_t n = pts.size();
    if (n < 12 || nrm.size() != n) return false;
    std::vector<V3> spine;
    spine.reserve(n);
    ...
    for (size_t i = 0; i + stride < n; ++i) {
        const V3& p1 = pts[i];      const V3& d1 = nrm[i];
        const V3& p2 = pts[i + stride]; const V3& d2 = nrm[i + stride];
        const double d1d2 = Dot(d1, d2);
        const double den = 1 - d1d2 * d1d2;
        if (den < 1e-4) continue;   /* near-parallel: no usable intersection */
        const V3 w = p1 - p2;
    ...
    if (spine.size() < 8) return false;
    double pl[4];
    if (!SeedPlane(spine, pl)) return false;
    const V3 a = Unit(V3(pl[0], pl[1], pl[2]));
    V3 c; Centroid(spine, c);
    ...
    R /= spine.size();
    if (!(R > 0)) return false;
    /* Minor radius: mean distance from the surface points to the spine circle. */
    double r = 0;
    for (const V3& p : pts) {
        const V3 v = p - c;
        const double u = Dot(v, a);
    ...
    r /= n;
    if (!(r > 0) || r >= R * 4) return false;
    q[0] = c.x; q[1] = c.y; q[2] = c.z;
    q[3] = a.x; q[4] = a.y; q[5] = a.z;
    q[6] = R; q[7] = r;
    return true;
}
...
{
    switch (k) {
        case kPlane: return "plane"; case kSphere: return "sphere";
        case kCylinder: return "cyl"; case kCone: return "cone";
        case kTorus: return "torus"; default: return "?";
    }
}
#else
...
Fit FitPatch(const PatchData& d, double tol, double scale)
{
    ...
    for (int i = 0; i < 5; ++i) {
        ...
        bool ok = false;
        switch (k) {
            case kPlane: ok = SeedPlane(pts, q); break;
            case kSphere: ok = SeedSphere(pts, q); break;
            case kCylinder: ok = SeedCylinder(d.spos, d.snrm, q); break;
            case kCone: ok = SeedCone(d.spos, d.snrm, q); break;
            case kTorus: ok = SeedTorus(d.spos, d.snrm, q); break;
            default: break;
        }
        if (!ok) continue;
        Renormalise(k, q);
...
#ifdef MESHRECON_TRACE
        std::fprintf(stderr, "      %-6s rms=%.6f tol=%.6f nrm=%.3f n=%d  "
                     "q=[%.3f %.3f %.3f %.3f %.3f %.3f %.4f %.4f]\n",
                     KindName(k), rms, tol,
                     NormalAgreement(k, q, d.spos, d.snrm, scale), (int)pts.size(),
        ...
        if (agree < kMinNormalAgreement) continue;
        if (rms <= tol) {
            best.kind = k;
    ...
    if (best.rms > tol) best.kind = kNone;   /* nothing fitted; caller decides */
    return best;
}
...
void PatchPoints(const Mesh& m, const std::vector<int>& tris, PatchData& d, int cap)
{
    d.pts.clear(); d.spos.clear(); d.snrm.clear();
    ...
    for (size_t i = 0; i < tris.size(); i += step) {
        ...
        for (int k = 0; k < 3; ++k) {
            ...
            if (seen.emplace(v, 1).second) d.pts.push_back(m.pos[v]);
        }
    }
}
                   ...
                   int& patchCount)
{
    ...
    for (int seed = 0; seed < nt; ++seed) {
        ...
        while (!stack.empty()) {
            ...
            for (int k = 0; k < 3; ++k) {
                const int o = m.adj[t * 3 + k];
                if (o < 0 || patchOf[o] >= 0) continue;
                if (Dot(m.tnorm[t], m.tnorm[o]) < cosSharp) continue;
                patchOf[o] = id;
                stack.push_back(o);
            }
        }
                ...
                double scale, int minTris, int origin, std::vector<Patch>& out)
{
    ...
    for (size_t i = 0; i < tris.size(); ++i) local.emplace(tris[i], static_cast<int>(i));
    ...
    for (size_t i = 0; i < tris.size(); ++i) {
        ...
        for (int k = 0; k < 3; ++k) {
            const int o = m.adj[t * 3 + k];
            if (o < 0 || local.find(o) == local.end()) continue;
            const double c = std::max(-1.0, std::min(1.0, Dot(m.tnorm[t], m.tnorm[o])));
    ...
    for (size_t i = 0; i < tris.size(); ++i) byArea[i] = static_cast<int>(i);
    std::sort(byArea.begin(), byArea.end(), [&](int a, int b) {
        if (std::fabs(bend[a] - bend[b]) > 1e-4) return bend[a] < bend[b];
    ...
    for (int si : byArea) {
        ...
        while (!frontier.empty()) {
            const int t = frontier.back(); frontier.pop_back();
            for (int k = 0; k < 3; ++k) {
                const int o = m.adj[t * 3 + k];
                if (o < 0) continue;
                auto it = local.find(o);
                if (it == local.end() || taken[it->second]) continue;
                bool accept;
                if (fit.kind == kNone) {
                     ...
                     * it cannot see, and MergeRegions repairs that afterwards. */
                    ...
                    for (int kk = 0; kk < 3 && accept; ++kk) {
                        ...
                        c += p;
                        if (std::fabs(SurfDist(fit.kind, fit.q, p)) > tol * 1.5) {
                            accept = false;
                        }
                    }
                    ...
                    if (accept) {
                        const V3 sn = SurfNormal(fit.kind, fit.q, c * (1.0 / 3.0),
                ...
                if (++sinceRefit >= std::max<int>(8, static_cast<int>(region.size()) / 4)) {
                    sinceRefit = 0;
                    PatchPoints(m, region, pd, 2000);
                    const Fit f = FitPatch(pd, tol, scale);
                    if (f.kind != kNone) fit = f;
                }
            }
        }
        ...
        if (pa.fit.kind == kNone &&
            static_cast<int>(region.size()) < minTris) {
                      ...
                      double scale, int passes)
{
    ...
    auto score = [&](int pi, int t) -> double {
        ...
        const double agree = (Norm(sn) > 0.5) ? std::fabs(Dot(sn, m.tnorm[t])) : 0.0;
    ...
    for (int pass = 0; pass < passes; ++pass) {
        ...
        for (int i = 0; i < n; ++i) {
            for (int t : patches[i].tris) patchOf[t] = i;
        }
        std::vector<std::pair<int, int>> moves;   /* (triangle, new patch) */
        std::vector<int> sizes(n);
        for (int i = 0; i < n; ++i) sizes[i] = static_cast<int>(patches[i].tris.size());

        for (int t = 0; t < m.triCount(); ++t) {
            const int own = patchOf[t];
            if (own < 0 || patches[own].fit.kind == kNone) continue;
            ...
            for (int k = 0; k < 3; ++k) {
                ...
                if (patches[q].origin < 0 || patches[q].origin != patches[own].origin) {
                    continue;
                }
                const double sc = score(q, t);
                if (sc < best) { best = sc; bestP = q; }
            }
        ...
        for (const std::pair<int, int>& mv : moves) patchOf[mv.first] = mv.second;
        std::vector<std::vector<int>> rebuilt(n);
        for (int t = 0; t < m.triCount(); ++t) {
            if (patchOf[t] >= 0) rebuilt[patchOf[t]].push_back(t);
        }
        for (int i = 0; i < n; ++i) {
            if (rebuilt[i].size() == patches[i].tris.size()) continue;
            patches[i].tris.swap(rebuilt[i]);
            if (patches[i].tris.empty()) continue;
            PatchPoints(m, patches[i].tris, pd, 4000);
            const Fit f = FitPatch(pd, tol, scale);
            if (f.kind != kNone) patches[i].fit = f;
        }
        std::vector<Patch> keep;
        keep.reserve(patches.size());
        for (Patch& pa : patches) {
            if (!pa.tris.empty()) keep.push_back(std::move(pa));
        }
        patches.swap(keep);
        if (static_cast<int>(patches.size()) != n) return;   /* sizes moved; stop */
    }
}
    ...
    for (int pass = 0; pass < maxPasses; ++pass) {
        ...
        std::vector<std::pair<int, int>> pairs;
        {
            ...
            for (long long k : seen) {
                pairs.emplace_back(static_cast<int>(k / n), static_cast<int>(k % n));
            }
        }
        if (pairs.empty()) return;
        /* Smallest first. */
        std::sort(pairs.begin(), pairs.end(), [&](const std::pair<int, int>& x,
        ...
        for (const std::pair<int, int>& pr : pairs) {
            ...
            if (mergedInto[a] >= 0 || mergedInto[b] >= 0) continue;
            ...
            if (patches[a].origin < 0 || patches[a].origin != patches[b].origin) {
                continue;
            }
            uni.clear();
            uni.reserve(patches[a].tris.size() + patches[b].tris.size());
            uni.insert(uni.end(), patches[a].tris.begin(), patches[a].tris.end());
            uni.insert(uni.end(), patches[b].tris.begin(), patches[b].tris.end());
            PatchPoints(m, uni, pd, 4000);
            const Fit f = FitPatch(pd, tol, scale);
            if (f.kind == kNone || f.rms > tol) continue;
            ...
            if (f.agree < kMergeNormalGate) continue;
        ...
        keep.reserve(patches.size());
        for (Patch& p : patches) {
            if (!p.tris.empty()) keep.push_back(std::move(p));
        }
        patches.swap(keep);
        if (!changed) return;
    }
                ...
                double tol, double scale)
{
    ...
    for (size_t i = 0; i < patches.size(); ++i) {
        if (patches[i].fit.kind != kNone) order.push_back(static_cast<int>(i));
    }
    ...
    for (int i : order) {
        Fit& f = patches[i].fit;
        const int dirOff = (f.kind == kPlane) ? 0
                         : (f.kind == kCylinder || f.kind == kCone || f.kind == kTorus) ? 3
                         : -1;
        if (dirOff < 0) continue;
        ...
        f.q[dirOff] = d.x; f.q[dirOff + 1] = d.y; f.q[dirOff + 2] = d.z;
        ...
        PatchPoints(m, patches[i].tris, pd, 4000);
        bool freeMask[8];
        for (int k = 0; k < 8; ++k) freeMask[k] = true;
    ...
    if (!(prm.snap_radius_frac > 0)) return;
    const double rTol = scale * prm.snap_radius_frac;
    std::vector<int> cyl;
    for (size_t i = 0; i < patches.size(); ++i) {
        if (patches[i].fit.kind == kCylinder) cyl.push_back(static_cast<int>(i));
    }
    ...
    for (size_t a = 0; a < cyl.size(); ++a) {
        ...
        const V3 axA = Unit(V3(patches[cyl[a]].fit.q[3], patches[cyl[a]].fit.q[4],
        ...
        for (size_t b = a + 1; b < cyl.size(); ++b) {
            ...
            if (std::fabs(Dot(axA, axB)) < cosTol) continue;
            if (std::fabs(fb.q[6] - patches[cyl[a]].fit.q[6]) > rTol) continue;
        ...
        for (int g : group) {
            ...
            bool freeMask[8] = {true, true, true, false, false, false, false, true};
            double q2[8];
            std::memcpy(q2, f.q, sizeof(q2));
            RefineFit(f.kind, q2, pd.pts, scale, freeMask);
            const double rms = FitRms(f.kind, q2, pd.pts);
...
Handle(Geom_Surface) MakeSurface(const Fit& f)
{
    try {
        switch (f.kind) {
            ...
            case kSphere: {
                if (!(f.q[3] > 0)) return nullptr;
                return new Geom_SphericalSurface(
                    gp_Ax3(gp_Pnt(f.q[0], f.q[1], f.q[2]), gp_Dir(0, 0, 1)), f.q[3]);
            }
            case kCylinder: {
                if (!(f.q[6] > 0)) return nullptr;
                return new Geom_CylindricalSurface(
                    gp_Ax3(gp_Pnt(f.q[0], f.q[1], f.q[2]),
                           gp_Dir(f.q[3], f.q[4], f.q[5])), f.q[6]);
            }
            case kCone: {
                /* OCCT wants the half-angle strictly inside (0, pi/2). */
                if (!(f.q[6] > 1e-4) || f.q[6] > M_PI / 2 - 1e-4) return nullptr;
                return new Geom_ConicalSurface(
                    gp_Ax3(gp_Pnt(f.q[0], f.q[1], f.q[2]),
                           gp_Dir(f.q[3], f.q[4], f.q[5])), f.q[6], 0.0);
            }
            case kTorus: {
                if (!(f.q[6] > 0) || !(f.q[7] > 0) || f.q[7] >= f.q[6]) return nullptr;
                return new Geom_ToroidalSurface(
                    gp_Ax3(gp_Pnt(f.q[0], f.q[1], f.q[2]),
                           gp_Dir(f.q[3], f.q[4], f.q[5])), f.q[6], f.q[7]);
            }
                 ...
                 std::vector<std::vector<Chain>>& loops)
{
    ...
    while (consumed < total) {
        ...
        int start = -1;
        for (auto& e : outgoing) {
            if (cursor[e.first] < e.second.size()) { start = e.first; break; }
        }
        if (start < 0) break;
        ...
        if (closedLoop) {
            size_t k = 0;
            for (; k < n; ++k) {
                if (loopOther[k] != loopOther[(k + n - 1) % n]) break;
            }
            begin = (k == n) ? 0 : k;   /* k==n: one neighbour all the way round */
        }
        ...
        if (closedLoop && chains.empty()) cur.closed = true;
        if (cur.verts.size() >= 2) chains.push_back(cur);
        if (!chains.empty()) loops.push_back(chains);
    }
}
 ...
 * tolerance, and on a model with ten thousand of them, some pair always fails. */
struct EdgeKey {
    int p1, p2, v1, v2, vm;
...
TopoDS_Vertex VertexAt(BuildCtx& ctx, int v)
{
    ...
    if (it != ctx.verts.end()) return it->second;
                                     ...
                                     const std::vector<gp_Pnt>& pts, double tol)
{
    ...
    try {
        ...
        if (!iss.IsDone() || iss.NbLines() < 1) return nullptr;
        ...
        for (int i = 1; i <= iss.NbLines(); ++i) {
            ...
            double err = 0;
            int n = 0;
            for (size_t k = 0; k < pts.size(); k += std::max<size_t>(1, pts.size() / 8)) {
                GeomAPI_ProjectPointOnCurve pp(pts[k], c);
                if (pp.NbPoints() < 1) { err = 1e300; break; }
        ...
        if (best.IsNull() || bestErr > tol * 3) return nullptr;
        return best;
    } catch (const Standard_Failure&) {
        return nullptr;
    }
                      ...
                      const std::vector<Handle(Geom_Surface)>& surfs)
{
    ...
    if (it != ctx.edges.end()) return it->second;

    std::vector<gp_Pnt> pts;
    pts.reserve(c.verts.size());
    for (int v : c.verts) pts.push_back(P(ctx.m->pos[v]));
                ...
                if (closed && cur->IsClosed()) {
                    ...
                    if (p1.NbPoints() > 0 && p2.NbPoints() > 0) {
                        ...
                        if (cur->IsPeriodic()) {
                            ...
                            GeomAPI_ProjectPointOnCurve pm(pts[pts.size() / 2], cur);
                            if (pm.NbPoints() > 0) {
                                double um = pm.LowerDistanceParameter();
                                while (u2 < u1) u2 += per;
                                while (um < u1) um += per;
                                if (um > u2) u2 -= per;   /* the other way round */
                            }
                        }
    ...
    if (e.IsNull() && c.verts.size() == 2) {
        try {
            BRepBuilderAPI_MakeEdge me(VertexAt(ctx, c.verts.front()),
                                       VertexAt(ctx, c.verts.back()));
            if (me.IsDone()) { e = me.Edge(); ctx.approximated++; }
        } catch (const Standard_Failure&) {
        }
    }
    ...
    if (!e.IsNull()) ctx.edges.emplace(key, e);
    return e;
}
                   ...
                   const Handle(Geom_Surface)& surf)
{
    ...
    for (size_t i = 0; i < tris.size(); i += step) {
        for (int k = 0; k < 3; ++k) vids.push_back(m.tri[tris[i] * 3 + k]);
    }
    ...
    auto span = [&](std::vector<double>& t, bool periodic, double lo, double hi,
        ...
        if (gap < per * 0.15) { a = lo; b = hi; full = true; return; }
        full = false;
        if (at == 0) { a = t.front(); b = t.back(); }
        else { a = t[at]; b = t[at - 1] + per; }
    };
    span(us, uPer, nu1, nu2, e.u1, e.u2, e.uFull);
    span(vs, vPer, nv1, nv2, e.v1, e.v2, e.vFull);
    if (!(e.u2 > e.u1) || !(e.v2 > e.v1)) return e;
    e.ok = true;
    return e;
}
                         ...
                         std::vector<TopoDS_Face>& out)
{
    try {
        ...
        if (!e.ok) {
            ...
            if (!mf.IsDone()) { MR_TRACE("      param: natural bounds failed\n"); return false; }
            face = mf.Face();
        } else {
            BRepBuilderAPI_MakeFace mf(surf, e.u1, e.u2, e.v1, e.v2, ctx.tol);
            if (!mf.IsDone()) {
                MR_TRACE("      param: bounds u[%.3f %.3f] v[%.3f %.3f] failed\n",
                         e.u1, e.u2, e.v1, e.v2);
                return false;
            }
            face = mf.Face();
                       ...
                       std::vector<TopoDS_Face>& out)
{
    ...
    MR_TRACE("      face %d: kind=%s loops=%d tris=%d\n", self,
             KindName(patch.fit.kind), (int)loops.size(), (int)patch.tris.size());

    /* No boundary at all means a closed surface the mesh wrapped completely —
     * a whole sphere, a whole torus. Only the parametric path can express it. */
    if (loops.empty()) return BuildParametricFace(ctx, m, patch, surf, out);
    ...
    for (const std::vector<Chain>& chains : loops) {
        if (chains.size() != 1 || chains[0].verts.front() != chains[0].verts.back()) {
            allClosed = false;
            break;
        }
    }
    if (allClosed && (surf->IsUPeriodic() || surf->IsVPeriodic())) {
        ...
        if (e.ok && (e.uFull || e.vFull)) {
            if (BuildParametricFace(ctx, m, patch, surf, out)) return true;
        }
    }
    ...
    try {
         ...
         * inner itself, which is the order these two steps have to happen in. */
        {
            BRep_Builder bb;
            for (size_t i = 1; i < wires.size(); ++i) bb.Add(face, wires[i]);
        }
                            ...
                            Report& rep)
{
    ...
    if (sewn.IsNull()) {
        ...
        TopoDS_Compound c;
        b.MakeCompound(c);
        for (const TopoDS_Face& f : faces) b.Add(c, f);
        return c;
    }
    ...
    try {
        ...
        fix->Perform();
        if (!fix->Shape().IsNull()) sewn = fix->Shape();
    } catch (const Standard_Failure&) {
    }
    ...
    if (!TopExp_Explorer(sewn, TopAbs_SHELL).More()) {
        ...
        if (!loose.empty()) {
            try {
                ...
                TopoDS_Shell sh;
                b.MakeShell(sh);
                for (const TopoDS_Face& f : loose) b.Add(sh, f);
    ...
    if (solids.empty() && open.size() == 1) return open[0];

    BRep_Builder b;
    TopoDS_Compound c;
    b.MakeCompound(c);
    for (const TopoDS_Solid& s : solids) b.Add(c, s);
    for (const TopoDS_Shape& s : open) b.Add(c, s);
    return c;
}
                         ...
                         const Params& prm, Report& rep, std::string& err)
{
    ClearReport(rep);
    err.clear();
    if (!xyz || !tri) { err = "no mesh data"; return TopoDS_Shape(); }
    ...
    try {
        ...
        err = f.GetMessageString() ? f.GetMessageString() : "mesh preparation failed";
        return TopoDS_Shape();
    }
    ...
    if (prm.mode == 0) {
        ...
        for (int i = 0; i < m.triCount(); ++i) all[i] = i;
        ...
        if (out.IsNull()) err = "the faces would not sew into a shell";
        else {
            ...
            try {
                ShapeUpgrade_UnifySameDomain uni(out, Standard_True, Standard_True,
                                                 Standard_False);
                uni.Build();
                if (!uni.Shape().IsNull()) out = uni.Shape();
            } catch (const Standard_Failure&) {
            }
        }
        return out;
    ...
    try {
        ...
        for (int t = 0; t < m.triCount(); ++t) byPatch[patchOf[t]].push_back(t);

        PatchData pd;
        for (int i = 0; i < rawPatches; ++i) {
            if (byPatch[i].empty()) continue;
            ...
            pa.fit = FitPatch(pd, tol, scale);
            if (pa.fit.kind != kNone ||
                static_cast<int>(byPatch[i].size()) < prm.min_patch_triangles * 4) {
                ...
                SplitByFit(m, byPatch[i], tol, scale, prm.min_patch_triangles, i,
                           patches);
                if (patches.size() == before) patches.push_back(pa);
            }
        }
        ...
        err = f.GetMessageString() ? f.GetMessageString() : "segmentation failed";
        return TopoDS_Shape();
    }
    ...
    for (size_t i = 0; i < patches.size(); ++i) {
        for (int t : patches[i].tris) patchOf[t] = static_cast<int>(i);
    }
    ...
    for (size_t i = 0; i < patches.size(); ++i) {
        ...
        if (surfs[i].IsNull()) continue;
        double area = 0;
        for (int t : patches[i].tris) area += m.tarea[t];
        rmsNum += patches[i].fit.rms * area;
        rmsDen += area;
        switch (patches[i].fit.kind) {
            case kPlane: rep.planes++; break;
            case kCylinder: rep.cylinders++; break;
            case kCone: rep.cones++; break;
            case kSphere: rep.spheres++; break;
            case kTorus: rep.tori++; break;
            default: break;
        }
    }
    ...
    for (size_t i = 0; i < patches.size(); ++i) {
        ...
        if (!surfs[i].IsNull()) {
            try {
                built = BuildAnalyticFace(ctx, m, patches[i], static_cast<int>(i),
                                          patchOf, surfs, faces);
            } catch (const Standard_Failure&) {
                built = false;
            }
        ...
        if (built) {
            ...
            if (!surfs[i].IsNull()) rep.faces_failed++;
            rep.faceted_patches++;
    ...
    for (const std::vector<int>& tris : deferred) EmitFaceted(m, tris, faces);
