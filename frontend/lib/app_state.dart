...
class AppState extends ChangeNotifier {
  ...
  /// [avoid] is the kept trim carrier (M125), which must never be mistaken for
  /// the cutter.
      ...
      for (final p in endIdx) {
          ...
          for (var pj = 0; pj < ptCount(gs[j]); pj++) {
            if ((getPt(gs[j], pj) - q).distance < tol) {
              ...
              if (wouldOverconstrain(gs, trial, pp)) {
                // M125 — with the carrier kept, a cut point is already pinned
        ...
        if (cand == null) {
          for (var j = 0; j < gs.length; j++) {
            ...
            // The kept CARRIER (M125) lies under every cut point by
  ...
  /// M125 — Trim that KEEPS the carrier. [old] stays where it was (index [i],
  ...
  void _bindPiecesToCarrier(
    ...
    void tryAdd(Constraint c) {
      ...
      Log.i('modify', 'carrier-bind ${conStr(-1, c)} (M125 trim carrier)');
      cons.add(c);
    }
  ...
  int? _pickEntity(SketchModel s, Offset w) {
    var best = -1;
    var bd = 10 / zoom;
    // A TIE goes to normal geometry. After a kept-carrier trim (M125) every
  ...
  void _modifyClick(SketchModel s, Offset w) {
    ...
    switch (tool) {
      case Tool.trim:
        ...
        // M125 — the CARRIER SURVIVES as construction geometry. Trimming used
