...
> **INTEGRATOR, at merge:** every scenario reference in this file was written
> as `[39x]` and has been renumbered to **`[40x]`**. S15 and S16 ran in
> parallel and both claimed scenario 39 in `smoke_occt.c` — the same collision
> class as the shim version number, one layer down. S15 merged first and kept
> 39; this session's twelve fixtures are 40a–40l. Only the labels moved:
> no fixture, assertion or number in this document changed.
...
fixture that demonstrates it is not.** [40j] now uses an **equilateral
...
from `backend/occt/build/occt_smoke`, scenario [40]; `occt_smoke` reports
...
```
[40d] ccw vol 2521.2203 z[-0.9968,50.9969] | cw vol 2521.2203 z[-50.9969,0.9968]
```
...
```
[40i] oblique move volume 10000.000000 ... valid=1, ray x=2 hits=2 0.0000 22.0000
```
...
```
[40j] theta=60 edge: mode2 alpha=80 -> built | mode2 alpha=100 -> REFUSED |
...
* **`occt_revolve_profile` is fully general in its axis.** `axis_side()` is a
  ...
  axis, on that axis reversed, on a quarter turn, and with a hole. [40b], [40c].
...
* **`occt_transform` has six orders of headroom.** A hand-composed Rodrigues
  ...
  exactly `0`/`±1`, so this had genuinely never been tested; it is fine. [40a].
...
* **Chamfer mode 1 works**, removes `½·d1·d2·L` exactly, and lands its two
  ...
  is swapped). It had **no fixture anywhere in the file** before this. [40l].
...
values of order 10, where 1e-7 is 1e-8 relative. [40a]'s round trip compared
...
* **Fixed nothing.** All three defects are behaviour changes; the brief and
  ...
  fixtures for the two that are silent ([40d], [40i]) **pin the defective
...
| New fixtures for every non-trivial case | **12**, scenario [40a]–[40l] |
